...
> 版本：v2.0（专家级补全版）  
> 仿真周期：60天  
> 数据规模：约 80 客户订单 / 270 生产任务 / 480 质检记录
...
| `daily_scheduler` | 每天09:00 | 运行APS排程（跳过周日） |
...
### 5.2 排程算法（daily_scheduler）

**关键逻辑流程**：
1. 查询前驱工序已完成、物料已就绪的"待排工序"
2. 按**关键比（Critical Ratio）**排序（剩余时间/剩余工期 < 1.1 的优先处理）
3. 对每道工序，遍历候选机台，计算 `score = 完工时间 + 换线时间×换线成本权重`
4. 选得分最低的机台，计算 `planned_start_time`（自动跳过周日 → `skip_sunday()`）
5. 写入 `production_task`，更新机台状态 `next_available_time`
