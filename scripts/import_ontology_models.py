...
ALL_FIELDS = {
    ...
    "machine": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "机台状态（枚举：在线/离线/维护中/故障/待机）", "is_enum": True, "enum_values": ["在线", "离线", "维护中", "故障", "待机"]},
    ...
    "customer_product": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "客户产品关系状态（枚举：有效/已停用）", "is_enum": True, "enum_values": ["有效", "已停用"]},
    ],
    "customer_order": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "订单状态（枚举：已确认/部分发货/已发货/已取消/重工中）", "is_enum": True, "enum_values": ["已确认", "部分发货", "已发货", "已取消", "重工中"]},
    ...
    "work_order": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "工单状态（枚举：已下达/已计划/进行中/生产中/已完成/已取消）", "is_enum": True, "enum_values": ["已下达", "已计划", "进行中", "生产中", "已完成", "已取消"]},
    ...
    "work_order_operation": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "工单工序状态（枚举：已完成/已排程/待开工/进行中/已取消）", "is_enum": True, "enum_values": ["已完成", "已排程", "待开工", "进行中", "已取消"]},
    ...
    "work_order_material": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "工单物料状态（枚举：待分配/已消耗/已齐套/已取消/部分分配/缺料）", "is_enum": True, "enum_values": ["待分配", "已消耗", "已齐套", "已齐套", "部分分配", "缺料"]},
        {"field_id": "note", "data_type": "text", "name": "备注", "required": False,  "description": "备注说明"},
    ],
    "purchase_order": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "采购订单状态（枚举：已入库/已创建/已到货）", "is_enum": True, "enum_values": ["已入库", "已创建", "已到货"]},
    ...
    "purchase_order_line": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "采购订单行状态（枚举：待收货/部分到货/全部到货）", "is_enum": True, "enum_values": ["待收货", "部分到货", "全部到货"]},
    ...
    "production_task": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "生产任务状态（枚举：已排程/待执行/执行中/已完成/已取消/已延期）", "is_enum": True, "enum_values": ["已排程", "待执行", "执行中", "已完成", "已取消", "已延期"]},
        {"field_id": "note", "data_type": "text", "name": "备注", "required": False,  "description": "备注说明"},
    ],
    "material_transfer": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": False,  "description": "调拨单状态（枚举：已执行）", "is_enum": True, "enum_values": ["已执行"]},
    ...
    "inventory_transaction": [
        ...
        {"field_id": "material_id", "data_type": "string", "name": "物料ID", "required": True,  "description": "物料唯一标识符"},
        {"field_id": "transaction_type", "data_type": "string", "name": "事务类型", "required": True,  "description": "事务类型（枚举：出库/IQC入库/取消释放/生产消耗/盘点亏损/盘点盈余/调拨出库/调拨入库/采购入库/预留/释放预留/生产入库）", "is_enum": True, "enum_values": ["出库", "IQC入库", "取消释放", "生产消耗", "盘点亏损", "盘点盈余", "调拨出库", "调拨入库", "采购入库", "预留", "释放预留", "生产入库"]},
    ...
    "quality_inspection": [
        ...
        {"field_id": "inspection_type", "data_type": "string", "name": "检验类型", "required": True,  "description": "检验类型（枚举：FQC出货检验/IQC入料/IQC来料检验/首件检验/过程质检）", "is_enum": True, "enum_values": ["FQC出货检验", "IQC入料", "IQC来料检验", "首件检验", "过程质检"]},
        {"field_id": "wo_op_id", "data_type": "string", "name": "工单工序ID", "required": False,  "description": "工单工序唯一标识符"},
        {"field_id": "lot_id", "data_type": "string", "name": "批次ID", "required": False,  "description": "批次唯一标识符"},
    ...
    "machine_status_log": [
        ...
        {"field_id": "status", "data_type": "string", "name": "状态", "required": True,  "description": "机台状态（枚举：恢复/换线/故障/空闲/维护/运行）", "is_enum": True, "enum_values": ["恢复", "换线", "故障", "空闲", "维护", "运行"]},
    ...
    "external_supply_chain_risk": [
        ...
        {"field_id": "risk_category", "data_type": "string", "name": "风险类别", "description": "风险类别（枚举：自然灾害/物流延期/原材料供应短缺/原材料价格波动/政治事件/财务风险/质量风险/法律风险/运营风险）", "required": True, "is_enum": True, "enum_values": ["自然灾害", "物流延期", "原材料供应短缺", "原材料价格波动", "政治事件", "财务风险", "质量风险", "法律风险", "运营风险"]},
        {"field_id": "risk_level", "data_type": "string", "name": "风险等级", "description": "风险等级（枚举：严重/高/中/低）", "required": True, "is_enum": True, "enum_values": ["严重", "高", "中", "低"]},
        ...
        {"field_id": "impact_scope", "data_type": "string", "name": "影响范围", "description": "影响范围（枚举：全球/区域/局部）", "required": False, "is_enum": True, "enum_values": ["全球", "区域", "局部"]},
        ...
        {"field_id": "status", "data_type": "string", "name": "处理状态", "description": "风险处理状态（枚举：新发现/待处理/分析中/缓解中/已解决/已忽略）", "required": False, "is_enum": True, "enum_values": ["新发现", "待处理", "分析中", "缓解中", "已解决", "已忽略"]},
    ...
    "supplier_risk_association": [
        ...
        {"field_id": "supplier_id", "data_type": "string", "name": "供应商ID", "required": True,  "description": "供应商唯一标识符"},
        {"field_id": "risk_id", "data_type": "string", "name": "风险ID", "required": True,  "description": "风险事件唯一标识符"},
        {"field_id": "association_type", "data_type": "string", "name": "关联类型", "description": "关联类型（枚举：直接/间接/潜在）", "required": False, "is_enum": True, "enum_values": ["直接", "间接", "潜在"]},
        {"field_id": "impact_level", "data_type": "string", "name": "影响程度", "description": "影响程度（枚举：严重/高/中/低）", "required": False, "is_enum": True, "enum_values": ["严重", "高", "中", "低"]},
        {"field_id": "note", "data_type": "text", "name": "备注", "required": False,  "description": "备注说明"},
		{"field_id": "created_at", "data_type": "datetime", "name": "创建时间", "required": False, "description": "记录创建时间"},
    ],
}
