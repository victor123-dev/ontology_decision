...
class Product(Base):
    ...
    product_type = Column(String(255), default="成品")
    standard_cycle_time = Column(Float, default=1.0)
    routing_steps = Column(Integer, default=3)
    setup_group = Column(String(255), default="DEFAULT")
    unit_of_measure = Column(String(255), default="PCS")
...
class Material(Base):
    ...
    material_type = Column(String(255), default="原材料")
    unit_of_measure = Column(String(255), default="PCS")
...
class BOM(Base):
    ...
    bom_id = Column(String(255), primary_key=True)
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    step_id = Column(String(255), ForeignKey("route_step.step_id"), nullable=True)
    quantity_per_unit = Column(Float, default=1.0)
    is_critical = Column(Boolean, default=True)
    consumption_pattern = Column(String(255), default="工序开始时消耗")
    version = Column(String(255), default="v1.0")
    effective_date = Column(Date, nullable=True)
    expiry_date = Column(Date, nullable=True)

class Supplier(Base):
    __tablename__ = "supplier"
    supplier_id = Column(String(255), primary_key=True)
    supplier_name = Column(String(255), nullable=False)
    supplier_type = Column(String(255), default="直供")
    country = Column(String(255), default="中国")
    industry_position = Column(String(255), default="")
...
class SupplierMaterial(Base):
    ...
    sm_id = Column(String(255), primary_key=True)
    supplier_id = Column(String(255), ForeignKey("supplier.supplier_id"), nullable=False)
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
...
class MaterialSubstitute(Base):
    ...
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    substitute_material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    substitute_priority = Column(Integer, default=1)
    quality_grade = Column(String(255), default="同等级")
    approval_status = Column(String(255), default="已批准")
...
class RouteStep(Base):
    ...
    route_id = Column(String(255), ForeignKey("process_route.route_id"), nullable=False)
    sequence_no = Column(Integer, nullable=False)
    step_name = Column(String(255), nullable=False)
    operation_type = Column(String(255), default="加工")
    standard_time_hours = Column(Float, default=1.0)
    machine_type_required = Column(String(255), nullable=False)
...
class WorkCenter(Base):
    ...
    work_center_id = Column(String(255), primary_key=True)
    work_center_name = Column(String(255), nullable=False)
    work_center_type = Column(String(255), nullable=False)
    capacity_uom = Column(String(255), default="小时")
    is_active = Column(Boolean, default=True)
...
class Machine(Base):
    ...
    work_center_id = Column(String(255), ForeignKey("work_center.work_center_id"), nullable=False)
    max_capacity_per_hour = Column(Float, default=100.0)
    status = Column(String(255), default="在线")
    current_product_id = Column(String(255), ForeignKey("product.product_id"), nullable=True)
    current_setup_group = Column(String(255), nullable=True)
...
class MachineCapability(Base):
    ...
    capability_id = Column(String(255), primary_key=True)
    machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=False)
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
...
class SetupMatrix(Base):
    ...
    machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=False)
    from_product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    to_product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    setup_time_minutes = Column(Integer, default=30)
    setup_type = Column(String(255), default="换模")
    is_active = Column(Boolean, default=True)
...
class ShiftPattern(Base):
    ...
    shift_id = Column(String(255), primary_key=True)
...
class WorkCalendar(Base):
    ...
    work_center_id = Column(String(255), ForeignKey("work_center.work_center_id"), nullable=False)
    shift_id = Column(String(255), ForeignKey("shift_pattern.shift_id"), nullable=False)
...
class Customer(Base):
    ...
    customer_level = Column(String(255), default="普通")  # VIP/重要/普通
    industry = Column(String(255), nullable=True)  # 行业类别
    credit_limit = Column(Float, default=0.0)  # 信用额度（万元）
    payment_terms = Column(String(255), default="月结30天")  # 付款条件
    contact_person = Column(String(255), nullable=True)  # 联系人
    contact_phone = Column(String(255), nullable=True)  # 联系电话
    contact_email = Column(String(255), nullable=True)  # 联系邮箱
    address = Column(Text, nullable=True)  # 地址
    country = Column(String(255), default="中国")  # 国家
    region = Column(String(255), default="大陆")  # 地区：大陆/台湾/欧美/亚太
    status = Column(String(255), default="活跃")  # 活跃/暂停/黑名单
    note = Column(Text, nullable=True)
...
class CustomerProduct(Base):
    ...
    customer_id = Column(String(255), ForeignKey("customer.customer_id"), nullable=False)
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    ...
    quality_level = Column(String(255), default="标准")  # 质量等级：标准/车规/工规
    status = Column(String(255), default="活跃")
...
class CustomerOrder(Base):
    ...
    customer_id = Column(String(255), ForeignKey("customer.customer_id"), nullable=False)
    customer_name = Column(String(255), nullable=False)
    customer_po_number = Column(String(255), nullable=True)  # 客户采购订单号
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    ...
    status = Column(String(255), default="已确认")
    shipping_address = Column(Text, nullable=True)  # 发货地址
    quality_requirement = Column(String(255), default="标准")  # 质量要求
    packaging_requirement = Column(String(255), nullable=True)  # 包装要求
    note = Column(Text, nullable=True)
...
class WorkOrder(Base):
    ...
    status = Column(String(255), default="已计划")
    priority = Column(Integer, default=5)
    setup_group = Column(String(255), nullable=True)
    current_step_id = Column(String(255), ForeignKey("route_step.step_id"), nullable=True)
...
class WorkOrderOperation(Base):
    ...
    wo_op_id = Column(String(255), primary_key=True)
    work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=False)
    step_id = Column(String(255), ForeignKey("route_step.step_id"), nullable=False)
    sequence_no = Column(Integer, nullable=False)
    ...
    assigned_machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=True)
    status = Column(String(255), default="待开工")
...
class WorkOrderMaterial(Base):
    ...
    wom_id = Column(String(255), primary_key=True)
    work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=False)
    wo_op_id = Column(String(255), ForeignKey("work_order_operation.wo_op_id"), nullable=True)
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    ...
    status = Column(String(255), default="待分配")
    note = Column(Text, nullable=True)
...
class PurchaseOrder(Base):
    ...
    status = Column(String(255), default="已创建")
...
class PurchaseOrderLine(Base):
    ...
    po_id = Column(String(255), ForeignKey("purchase_order.po_id"), nullable=False)
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    quantity = Column(Float, nullable=False)
    unit_price = Column(Float, default=0.0)
    received_quantity = Column(Float, default=0.0)
    status = Column(String(255), default="未开始")
    related_work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)
    related_wom_id = Column(String(255), ForeignKey("work_order_material.wom_id"), nullable=True)
...
class WIPLot(Base):
    ...
    lot_id = Column(String(255), primary_key=True)
    work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=False)
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    lot_size = Column(Float, default=25, comment="批次大小（标准25片/批）")
    current_step_id = Column(String(255), ForeignKey("route_step.step_id"), nullable=True)
    current_machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=True)
    lot_quantity = Column(Float, nullable=False)
    actual_quantity = Column(Float, nullable=True)         # P0-2: 经良率损耗后的实际数量
    lot_status = Column(String(255), default="排队中")
...
class ProductionTask(Base):
    ...
    wo_op_id = Column(String(255), ForeignKey("work_order_operation.wo_op_id"), nullable=False)
    work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=False)
    machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=False)
    lot_id = Column(String(255), ForeignKey("wip_lot.lot_id"), nullable=True)  # P1-6: 关联WIPLot
    ...
    shift_id = Column(String(255), ForeignKey("shift_pattern.shift_id"), nullable=True)  # P2-10: 班次
    is_night_shift = Column(Boolean, default=False)  # P2-10: 是否夜班（用于效率折减）
    status = Column(String(255), default="已排程")
    note = Column(Text, nullable=True)
...
class MaterialTransfer(Base):
    ...
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    from_work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)  # Task11: 仓库间调拨时可为NULL
    to_work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)    # Task11: 仓库间调拨时可为NULL
    from_location = Column(String(255), nullable=True)  # Task11: 来源仓库
    to_location = Column(String(255), nullable=True)    # Task11: 目标仓库
    from_wom_id = Column(String(255), ForeignKey("work_order_material.wom_id"), nullable=True)
    to_wom_id = Column(String(255), ForeignKey("work_order_material.wom_id"), nullable=True)
    quantity = Column(Float, nullable=False)
    transfer_reason = Column(String(255), default="缺料挪用")
    trigger_source = Column(String(255), default="MRP运算")
...
class Inventory(Base):
    ...
    inventory_id = Column(String(255), primary_key=True)
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=False)
    location = Column(String(255), default="主仓库")
...
class InventoryTransaction(Base):
    ...
    related_document_type = Column(String(255), nullable=True)
    related_document_id = Column(String(255), nullable=True)
    from_work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)
    to_work_order_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)
...
class MachineStatusLog(Base):
    ...
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=True)
    running_wo_id = Column(String(255), ForeignKey("work_order.work_order_id"), nullable=True)
    running_task_id = Column(String(255), ForeignKey("production_task.task_id"), nullable=True)
...
class Schedule(Base):
    ...
    bottleneck_machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=True)
    bottleneck_work_center_id = Column(String(255), ForeignKey("work_center.work_center_id"), nullable=True)
...
class FinishedGoodsInventory(Base):
    ...
    fg_inv_id = Column(String(255), primary_key=True)
    product_id = Column(String(255), ForeignKey("product.product_id"), nullable=False)
    location = Column(String(255), default="成品仓")
    total_quantity = Column(Float, default=0.0)
    available_quantity = Column(Float, default=0.0)
...
class QualityInspection(Base):
    ...
    inspection_id = Column(String(255), primary_key=True)
    inspection_type = Column(String(255), nullable=False)   # "过程质检" / "IQC入料" / "完工检验" / "首件检验" / "FQC出货检验"
    wo_op_id = Column(String(255), ForeignKey("work_order_operation.wo_op_id"), nullable=True)
    lot_id = Column(String(255), ForeignKey("wip_lot.lot_id"), nullable=True)
    machine_id = Column(String(255), ForeignKey("machine.machine_id"), nullable=True)
    po_id = Column(String(255), ForeignKey("purchase_order.po_id"), nullable=True)  # IQC用
    material_id = Column(String(255), ForeignKey("material.material_id"), nullable=True)  # IQC用
    # Task8/9/11: 通用关联字段（支持IPQC、FQC等新类型）
    related_doc_type = Column(String(255), nullable=True)   # WorkOrderOperation/CustomerOrder/PurchaseOrder
    related_doc_id = Column(String(255), nullable=True)     # 对应文档ID
    ...
    result = Column(String(255), default="合格")  # 合格/返工/报废/让步接收/拒收
    disposition = Column(String(255), nullable=True)  # 处置说明
    is_hold = Column(Boolean, default=False)     # 是否处于Hold状态
    inspector = Column(String(255), default="QC-AUTO")
    note = Column(Text, nullable=True)
...
class ExternalSupplyChainRisk(Base):
    ...
    risk_id = Column(String(255), primary_key=True)  # 风险事件ID
    
    # 关联对象
    supplier_id = Column(String(255), ForeignKey("supplier.supplier_id"), nullable=True)  # 关联供应商
    customer_id = Column(String(255), ForeignKey("customer.customer_id"), nullable=True)  # 关联客户
    ...
    # 风险详情
    title = Column(String(255), nullable=False)  # 风险事件标题
    description = Column(Text, nullable=False)  # 风险事件描述
    source_url = Column(Text, nullable=True)  # 信息来源URL
    source_name = Column(String(255), nullable=True)  # 信息来源名称（如：Reuters, Bloomberg）
    
    # 影响评估
    impact_scope = Column(String(255), nullable=True)  # 影响范围：全球，区域，局部
    estimated_impact_days = Column(Integer, nullable=True)  # 预估影响天数
    affected_materials = Column(JSON, nullable=True)  # 受影响的物料（JSON数组）
    affected_products = Column(JSON, nullable=True)  # 受影响的产品（JSON数组）
    ...
    keywords = Column(JSON, nullable=True)  # 关键词（JSON数组）
...
class SupplierRiskAssociation(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    supplier_id = Column(String(255), ForeignKey("supplier.supplier_id"), nullable=False)
    risk_id = Column(String(255), ForeignKey("external_supply_chain_risk.risk_id"), nullable=False)
    association_type = Column(String(255), default="direct")  # 关联类型：直接, 间接, 潜在
    impact_level = Column(String(255), nullable=True)  # 对该供应商的影响程度：严重, 高, 中, 低
    note = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
