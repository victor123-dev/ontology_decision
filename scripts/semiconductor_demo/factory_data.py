...
# ============================================================================
# 通用工序类型定义 (短代码 -> 工作中心, 标准时间小时, 良率, 工序后等待时间, 转运时间)
# 修正：改为Lot批量加工时间（lot_size=25），贴合OSAT真实生产模式
# wait_time: 工序后等待时间（如固化、烘烤），不占机台，异步等待
# transport_time: 工序间转运时间，占机台（加工前等待转运到位）
PT = {
    #                          工作中心      加工时间  良率   等待时间  转运时间
    # 前段（批量加工，时间÷25）
    "RECV":  ("WC-RECV",     0.5,  1.00,   0.0,    0.05),  # 收货是批次操作
    "GRIND": ("WC-GRIND",    0.08, 0.99,   0.0,    0.05),  # 2.0/25=0.08
    "DICE":  ("WC-DICE",     0.06, 0.98,   0.0,    0.05),  # 1.5/25=0.06
    "DA":    ("WC-DA",       0.04, 0.99,   0.0,    0.05),  # 1.0/25=0.04
    "CURE":  ("WC-DA",       0.08, 1.00,   0.5,    0.05),  # 2.0/25=0.08，固化等待0.5h
    # 中段（键合是逐个的，但机台多线并行）
    "WB":    ("WC-WB",       0.4,  0.97,   0.0,    0.05),  # 2.0/5线=0.4（5线并行）
    "FC":    ("WC-FC",       0.3,  0.98,   0.0,    0.05),  # 1.5/5=0.3
    "RDL":   ("WC-RDL",      0.4,  0.98,   0.0,    0.05),  # 2.0/5=0.4
    # 塑封（批量）
    "MOLD":  ("WC-MOLD",     0.12, 0.98,   0.0,    0.05),  # 3.0/25=0.12
    "PMC":   ("WC-PMC",      0.16, 1.00,   0.5,    0.05),  # 4.0/25=0.16，后固化等待0.5h
    "MARK":  ("WC-MARK",     0.01, 1.00,   0.0,    0.05),  # 0.3/25=0.012
    # 植球（批量）
    "BALL":  ("WC-BALL",     0.06, 0.99,   0.0,    0.05),  # 1.5/25=0.06
    "REFL":  ("WC-BALL",     0.04, 1.00,   0.25,   0.05),  # 1.0/25=0.04，回流等待0.25h
    "SING":  ("WC-SING",     0.04, 0.99,   0.0,    0.05),  # 1.0/25=0.04
    # 检测（逐件但高速）
    "AOI":   ("WC-AOI",      0.02, 1.00,   0.0,    0.05),  # 0.5/25=0.02
    "VISUAL":("WC-VISUAL",   0.02, 0.99,   0.0,    0.05),  # 0.5/25=0.02
    "XRAY":  ("WC-XRAY",     0.02, 1.00,   0.0,    0.05),  # 0.5/25=0.02
    "SAM":   ("WC-SAM",      0.04, 1.00,   0.0,    0.05),  # 1.0/25=0.04
    # 测试（并行测试）
    "CP":    ("WC-TEST-CP",  0.4,  0.95,   0.0,    0.05),  # 2.0/5并行=0.4
    "DC":    ("WC-TEST-FT",  0.3,  0.96,   0.0,    0.05),  # 1.5/5=0.3
    "AC":    ("WC-TEST-FT",  0.3,  0.96,   0.0,    0.05),  # 1.5/5=0.3
    "FUNC":  ("WC-TEST-FT",  0.4,  0.95,   0.0,    0.05),  # 2.0/5=0.4
    "BI":    ("WC-TEST-BI",  0.96, 0.98,   0.5,    0.05),  # 24.0/25并行=0.96，老化等待0.5h
    "BIT":   ("WC-TEST-FT",  0.4,  0.97,   0.0,    0.05),  # 2.0/5=0.4
    "FINAL": ("WC-TEST-FT",  0.1,  0.97,   0.0,    0.05),  # 2.5/25=0.1
    "SYS":   ("WC-TEST-SYS", 0.8,  0.95,   0.0,    0.05),  # 4.0/5=0.8
    # 辅助（批量）
    "CLEAN": ("WC-CLEAN",    0.01, 1.00,   0.0,    0.05),  # 0.3/25=0.012
    "DRY":   ("WC-CLEAN",    0.02, 1.00,   0.25,   0.05),  # 0.5/25=0.02，干燥等待0.25h
    "BAKE":  ("WC-BAKE",     0.16, 1.00,   0.25,   0.05),  # 4.0/25=0.16，烘烤等待0.25h
    "WAIT":  ("WC-WAIT",     0.04, 1.00,   0.0,    0.05),  # 1.0/25=0.04
    "TRANS": ("WC-TRANS",    0.01, 1.00,   0.0,    0.0),   # 0.2/25=0.008，转运本身无转运时间
    # 出货（批量）
    "OQC":   ("WC-PACK",     0.02, 0.99,   0.0,    0.05),  # 0.5/25=0.02
    "PACK":  ("WC-PACK",     0.02, 1.00,   0.0,    0.05),  # 0.5/25=0.02
    "LABEL": ("WC-PACK",     0.01, 1.00,   0.0,    0.05),  # 0.2/25=0.008
    "STORE": ("WC-PACK",     0.01, 1.00,   0.0,    0.0),   # 0.2/25=0.008，最终入库无转运
}
...
def generate_routes_and_steps() -> Tuple[List[Dict], List[Dict]]:
    ...
    for pkg, cfg in PKG_CFG.items():
        ...
        for chip in CHIP_TYPES:
            ...
            seq = 10
            for code in flow:
                wc, time, yield_rate, wait_time, transport_time = PT[code]
        ...
        for chip in CHIP_TYPES:
            ...
            for m in MACHINES:
                ...
                caps.append({
                    ...
                    "yield_rate": round(0.97 + (hash(pid + mid) % 7) / 100, 2),  # 偏差因子：0.97~1.03（1.0=达标）
...
SUPPLIERS = [
    ...
    {
        "supplier_id": "SUP-001",
        "supplier_name": "欣兴电子（Unimicron）",
        "supplier_type": "战略合作",
        "country": "中国台湾",
        "industry_position": "全球IC载板Top 3，市占率15%+",
    ...
    {
        "supplier_id": "SUP-002",
        "supplier_name": "住友电木（Sumitomo Bakelite）",
        "supplier_type": "直供",
        "country": "日本",
        "industry_position": "全球环氧塑封料No.1，市占率25%+",
    ...
    {
        "supplier_id": "SUP-003",
        "supplier_name": "田中贵金属（Tanaka）",
        "supplier_type": "进口",
        "country": "日本",
        "industry_position": "全球键合线Top 2，日本市占率30%",
    ...
    {
        "supplier_id": "SUP-004",
        "supplier_name": "贺利氏（Heraeus）",
        "supplier_type": "战略合作",
        "country": "德国",
        "industry_position": "全球键合线No.1，市占率20%+",
    ...
    {
        "supplier_id": "SUP-005",
        "supplier_name": "飞凯材料（大瑞科技）",
        "supplier_type": "直供",
        "country": "中国",
        "industry_position": "国内锡球No.1，全球市占率10%+",
    ...
    {
        "supplier_id": "SUP-006",
        "supplier_name": "深南电路（SCC）",
        "supplier_type": "备选",
        "country": "中国",
        "industry_position": "国内IC载板No.1，FCBGA载板突破中",
        "avg_lead_time_days": 10,
    ...
    },
    {
        ...
        "supplier_type": "备选",
        "country": "中国",
        "industry_position": "国内环氧塑封料龙头，GMC产品通过验证",
    ...
    },
    {
        ...
        "supplier_type": "备选",
        "country": "中国",
        "industry_position": "国内FCBGA封装基板领先，AI驱动需求",
        "avg_lead_time_days": 12,
    ...
    {
        "supplier_id": "SUP-009",
        "supplier_name": "日立化成（Hitachi Chemical）",
        "supplier_type": "备选",
        "country": "日本",
        "industry_position": "全球塑封料Top 3，先进封装材料领先",
    ...
    {
        "supplier_id": "SUP-010",
        "supplier_name": "铨宝工业（Chua Kok）",
        "supplier_type": "备选",
        "country": "新加坡",
        "industry_position": "东南亚锡球主要供应商",
    ...
    },
    {
        ...
        "supplier_type": "备选",
        "country": "中国",
        "industry_position": "国内底部填充胶No.1，DAF材料领先",
    ...
    {
        "supplier_id": "SUP-012",
        "supplier_name": "揖斐电（Ibiden）",
        "supplier_type": "备选",
        "country": "日本",
        "industry_position": "全球IC载板No.1，市占率20%+",
    ...
    },
    {
        ...
        "supplier_type": "备选",
        "country": "中国",
        "industry_position": "球形硅微粉全球市占率领先，Chiplet封装材料",
        "avg_lead_time_days": 7,
...
def generate_supplier_materials() -> List[Dict]:
    ...
    # SUP-001 欣兴电子（Unimicron）：IC载板专业供应商
